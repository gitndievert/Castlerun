...
namespace MTAssets
{
    public class MeshCombinerTool : EditorWindow
    {
        ...
        */
        ...
        Vector2 scrollPos;
        string messageOfButton = "Combine Meshes Now!";
        int startMerge = 0;
        ...
        private bool lightmapSupport = false;
        private bool moreThan65kVertices = false;
        ...
        void OnGUI()
        {
            ...
            if (verticesCountInSelection <= 65000 || moreThan65kVertices == true)
            {
                ...
                if (listOfInvalidGameObjects.Count == 0)
                {
                    ...
                    if (transformsToCombine.Count >= 2)
                    {
                        ...
                        lightmapSupport = (bool)EditorGUILayout.Toggle(new GUIContent("Lightmap Support",
                        ...
                        moreThan65kVertices = (bool)EditorGUILayout.Toggle(new GUIContent("More Than 65k Vertices",
                                    "If this option is enabled, the 65,000 vertex limit is removed and you can merge meshes without worrying about the 65,000 vertex limitation imposed on common meshes. Keep in mind, that the more vertices a mesh has, the more memory and bandwidth it consumes in the GPU."),
                                    moreThan65kVertices);
                        ...
                        if (GUILayout.Button(messageOfButton, GUILayout.Height(40)))
                        {
                            //Update the text of button
                            messageOfButton = "Combining, please, wait...";
                            if(startMerge < 1)
                            {
                                startMerge = 1;
                            }
                        ...
                        //Starts the merge after 5 updates in interface
                        if (startMerge >= 1)
                        {
                            startMerge += 1;
                            if(startMerge == 5)
            ...
            if (verticesCountInSelection > 65000 && moreThan65kVertices == false)
            {
                GUILayout.EndVertical();
                EditorGUILayout.HelpBox("Oops! It looks like the 65.000 vertex limit for the selected GameObjects has been reached. If you combine meshes and the result mesh has more than 65.000 vertices, it may contain artifacts. Please unselect object by object until the vertex counter is below 65.000.\n\nVertices count of selected GameObjects: " + verticesCountInSelection.ToString() + "\n\nIf it is an option for you, you can enable the \"More Than 65k Vertices\" option to remove the 65,000 vertices limitation.", MessageType.Error);
        ...
        void CombineNow_OneMeshPerMaterial()
        {
            ...
            foreach (var key in meshesPerMaterial)
            {
                ...
                //Create the final mesh, and set limitation of vertices
                Mesh mesh = new Mesh();
                ...
                mesh.CombineMeshes(combineInstances.ToArray(), true, true, lightmapSupport);

                //Create the GameObject to store this mesh, and give support for lightmaps
